...
class PaypalPaymentTest(test.APISimpleTestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('paypal-payment-list')
    ...
    def approve_payment(self, user):
        with mock.patch('paypalrestsdk.Payment', self.mock):
            ...
            self.client.force_authenticate(user)
            base_url = reverse('paypal-payment-detail', kwargs={'uuid': payment.uuid.hex})
    ...
    def cancel_payment(self, user):
        with mock.patch('paypalrestsdk.Payment', self.mock):
            ...
            self.client.force_authenticate(user)
            url = reverse('paypal-payment-detail', kwargs={'uuid': payment.uuid.hex}) + 'cancel/'
            return self.client.get(url)
    ...
    def test_when_payment_created_correct_database_object_created(self):
        ...
        payment = Payment.objects.get(backend_id=self.response_data['payment_id'])
    ...
    def test_user_can_approve_payment_for_owned_customer(self):
        ...
        self.assertEqual(response['Location'], PaypalBackend().return_url)
